...
const fetchApi = async () => {
    try {
        loading.value = true;
        const responseData = await fetch("http://localhost:7000/api/login", {
            method: "POST",
            headers: {
    ...
    <form class="w-[90%] p-5 sm:w-[450px] m-auto shadow-xl">
        ...
        <div class="space-y-3">
            ...
            <label class="flex flex-col">
                <span class="text-[16px] lg:text-lg font-normal">Password</span>
                <input
                    type="password"
                    name="password"
                    v-model="password"
        ...
        <div class="mt-2 md:mt-4 flex justify-center items-center gap-1">
            <span class="text-md md:text-[16px]">Don't have an account ?</span>
            <RouterLink
                to="/auth/register"
